...
ejecutar_y_mostrar <- function(ahorros_actuales_usd, salario_mensual_usd, tasa_ahorro_pct,
 ...
 params <- construir_params(ahorros_actuales_usd, salario_mensual_usd, tasa_ahorro_pct,
  ...
  cat("================================================================\n")
  cat("              RESUMEN RAPIDO (cuentas simples, sin simulacion)\n")
  cat("================================================================\n\n")
  cat("   Estos numeros NO incluyen crisis, perdida de empleo, ni\n")
  cat("   variaciones del mercado: son la cuenta directa si todo\n")
  cat("   se mantiene estable durante todo el periodo.\n\n")
  cat("   COMPRANDO:\n")
  cat(sprintf("      Pago inicial (anticipo + escritura): $%s USD\n",
              format(round(costo_inicial_compra), big.mark = ",")))
  cat(sprintf("      Cuota mensual del credito:           $%s USD/mes\n",
              format(round(cuota_mensual_credito), big.mark = ",")))
  cat(sprintf("      Total pagado en %d anos de cuotas:    $%s USD\n",
              plazo_anos, format(round(total_pagado_credito), big.mark = ",")))
  cat("      Al terminar de pagar, la propiedad es tuya.\n\n")
  cat("   ALQUILANDO:\n")
  cat(sprintf("      Pago inicial (deposito + comision):  $%s USD\n",
              format(round(costo_inicial_alquiler), big.mark = ",")))
  cat(sprintf("      Alquiler mensual:                    $%s USD/mes\n",
              format(round(alquiler_mensual_usd), big.mark = ",")))
  cat(sprintf("      Total pagado en %d anos de alquiler:  $%s USD\n",
              plazo_anos, format(round(total_pagado_alquiler), big.mark = ",")))
  cat("      Al terminar, no sos dueno de nada, pero pudiste\n")
  cat("      invertir lo que no gastaste de entrada ni en cuotas.\n\n")
  ...
  if (prob_compra_gana > 0.6) {
    ...
    cat("------------------------------------------------------------\n")
    cat(sprintf("\n   Terminarias con aproximadamente $%s USD mas\n", format(round(abs(diferencia_media)), big.mark=",")))
    cat(sprintf("   Comprar te conviene en %.0f de cada 100 escenarios probados\n\n", prob_compra_gana * 100))
  } else if (prob_compra_gana < 0.4) {
    cat("------------------------------------------------------------\n")
    cat("       RECOMENDACION: ALQUILAR                              \n")
    cat("------------------------------------------------------------\n")
    cat(sprintf("\n   Terminarias con aproximadamente $%s USD mas\n", format(round(abs(diferencia_media)), big.mark=",")))
    cat(sprintf("   Alquilar te conviene en %.0f de cada 100 escenarios probados\n\n", (1-prob_compra_gana) * 100))
  } else {
    cat("------------------------------------------------------------\n")
    cat("       RESULTADO: AMBAS OPCIONES SON SIMILARES              \n")
    cat("------------------------------------------------------------\n")
    cat(sprintf("\n   Comprar te conviene en %.0f de cada 100 escenarios probados\n", prob_compra_gana * 100))
    cat(sprintf("   Alquilar te conviene en %.0f de cada 100 escenarios probados\n\n", (1-prob_compra_gana) * 100))
  }

  cat("------------------------------------------------------------\n")
  cat("                 TU CONFIGURACION\n")
  cat("------------------------------------------------------------\n")
...
<div class="slider-group">
<label>% del salario que podes ahorrar <span class="slider-value" id="val-tasa_ahorro">50%</span></label>
<input type="range" id="sl-tasa_ahorro" min="0" max="100" step="1" value="50">
<span class="hint">Lo que te queda libre cada mes para destinar a vivienda o ahorro</span>
</div>
...
<span class="hint">Meses de alquiler que pedis como deposito al firmar</span>
</div>
...
<span class="hint">Ganancia anual esperada si invertis el dinero que no usaste para comprar</span>
</div>
...
(function () {
  ...
  // heading / divider / plain line so it can be styled like the rest of the page.
