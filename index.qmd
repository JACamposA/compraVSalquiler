...
(function () {
  ...
  function setEditorContent(text) {
    const wrapper = document.getElementById("sim-app");
    const cmContent = wrapper && wrapper.querySelector(".cm-content");
  ...
  function clickRun() {
    ...
    for (const btn of buttons) {
      if (/run/i.test(btn.textContent || "")) { btn.click(); return true; }
    }
  ...
  btn.addEventListener("click", function () {
    ...
    const ok = setEditorContent(code);
    if (!ok) {
    ...
    setTimeout(function () {
      const ran = clickRun();
      status.textContent = ran ? "Resultados generados." : "Abre 'Motor de simulacion' y presiona Run Code manualmente.";
      btn.disabled = false;
    }, 150);
  });
})();
