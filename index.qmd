...
subtitle: "Comprar con UVA vs Alquilar - Analisis Monte Carlo"
format: 
  html:
    theme: flatly
    toc: false
...
<div class="app-hero">
  ...
  <p>Simulador Monte Carlo para Argentina — ajusta los controles y descubre que conviene mas en tu caso.</p>
</div>
```
...
ejecutar_y_mostrar <- function(ahorros_actuales_usd, salario_mensual_usd, tasa_ahorro_pct,
 ...
 params <- construir_params(ahorros_actuales_usd, salario_mensual_usd, tasa_ahorro_pct,
  ...
  tasa_real_mensual <- (1 + tasa_real_uva_anual)^(1/12) - 1
  ...
  total_pagado_credito <- cuota_mensual_credito * n_pagos
  ...
  cat(sprintf(
    "@@RESUMEN@@plazo=%d|compra_inicial=%.0f|compra_cuota=%.0f|compra_total=%.0f|alquiler_inicial=%.0f|alquiler_cuota=%.0f|alquiler_total=%.0f@@\n",
    plazo_anos, costo_inicial_compra, cuota_mensual_credito, total_pagado_credito,
  ...
  # Display results
  cat("================================================================\n")
  cat("           RESULTADOS DE LA SIMULACION (con riesgos e incertidumbre)\n")
  cat("================================================================\n\n")
...
<span class="hint">% del valor de la propiedad que pagas de entrada (el resto es el credito UVA)</span>
</div>

<div class="slider-group">
<label>Plazo del credito <span class="slider-value" id="val-plazo">25 años</span></label>
<input type="range" id="sl-plazo" min="5" max="35" step="1" value="25">
<span class="hint">Duracion del credito UVA</span>
</div>
...
<span class="hint">Tasa real anual del credito</span>
</div>
...
<div class="slider-group">
<label>Deposito en garantia <span class="slider-value" id="val-deposito">3</span></label>
<input type="range" id="sl-deposito" min="0" max="6" step="1" value="3">
<span class="hint">Meses de alquiler que te piden como deposito al firmar</span>
</div>

<div class="slider-group">
<label>Comision inmobiliaria <span class="slider-value" id="val-comision">1 mes</span></label>
<input type="range" id="sl-comision" min="0" max="3" step="0.5" value="1">
<span class="hint">Meses de comision al alquilar</span>
</div>
...
<span class="hint">Por año, por encima de inflacion</span>
</div>
...
<div class="slider-group">
<label>Precision del calculo <span class="slider-value" id="val-nsim">500</span></label>
<input type="range" id="sl-nsim" min="50" max="1000" step="50" value="500">
<span class="hint">Mas alto = resultado mas confiable, pero tarda un poco mas</span>
</div>
...
<span class="hint">CABA: 3% apreciacion anual | GBA: 1%</span>
</div>
...
(function () {
    ...
    return (
      '<div class="rs-card-grid">' +
        '<div class="rs-card rs-card-compra">' +
          ...
          '<div class="rs-card-row"><span>Cuota mensual del credito</span><strong>' + usd(f.compra_cuota) + "/mes</strong></div>" +
          '<div class="rs-card-row rs-card-row-total"><span>Total pagado en ' + f.plazo + ' anos</span><strong>' + usd(f.compra_total) + "</strong></div>" +
          '<div class="rs-card-note">Al terminar de pagar, la propiedad es tuya.</div>' +
        "</div>" +
        '<div class="rs-card rs-card-alquiler">' +
          '<div class="rs-card-title">Alquilando</div>' +
          '<div class="rs-card-row"><span>Pago inicial (deposito + comision)</span><strong>' + usd(f.alquiler_inicial) + "</strong></div>" +
          '<div class="rs-card-row"><span>Alquiler mensual</span><strong>' + usd(f.alquiler_cuota) + "/mes</strong></div>" +
          '<div class="rs-card-row rs-card-row-total"><span>Total pagado en ' + f.plazo + ' anos</span><strong>' + usd(f.alquiler_total) + "</strong></div>" +
          '<div class="rs-card-note">No sos dueno de nada, pero pudiste invertir lo que no gastaste.</div>' +
        "</div>" +
      "</div>"
    );
  }
  ...
  function renderRecomendacionCard(f) {
    const labels = { comprar: "RECOMENDACION: COMPRAR", alquilar: "RECOMENDACION: ALQUILAR", empate: "AMBAS OPCIONES SON SIMILARES" };
    ...
    const detalle = tipo === "empate"
      ...
      : "Terminarias con aproximadamente " + usd(f.diferencia) + " mas. Te conviene en " + f.prob + " de cada 100 escenarios probados.";
    return (
  ...
  function renderConfiguracionCard(f) {
    return (
      '<div class="rs-config-card">' +
        ...
        '<div class="rs-config-item"><span>Credito</span><strong>' + usd(f.credito) + " a " + f.plazo + " anos</strong></div>" +
        '<div class="rs-config-item"><span>Alquiler</span><strong>' + usd(f.alquiler) + "/mes</strong></div>" +
        '<div class="rs-config-item"><span>Ubicacion</span><strong>' + f.ubicacion + "</strong></div>" +
      "</div>"
    );
  }
  ...
  function renderPatrimonioCard(f) {
    ...
    return (
      '<div class="rs-section-label">Patrimonio estimado en ' + f.plazo + " anos</div>" +
  ...
  btn.addEventListener("click", async function () {
    btn.disabled = true;
    status.textContent = "Cargando motor de simulacion...";
    ...
    if (!entry || typeof window.qwebrExecuteCode !== "function") {
      status.textContent = "El motor aun esta cargando, intenta de nuevo en unos segundos.";
      btn.disabled = false;
      return;
    }
    ...
    try {
      ...
      status.textContent = "Ocurrio un error al calcular. Revisa 'Motor de simulacion'.";
      console.error(e);
    } finally {
      btn.disabled = false;
    }
...
**Inversiones:** Quien alquila invierte el dinero que no gasto en la compra en un fondo diversificado de acciones (similar al indice S&P 500 de EE.UU.), con un retorno esperado configurable y altibajos realistas. Incluye comportamiento de panico en caidas (vender en el peor momento).
