...
ejecutar_y_mostrar <- function(ahorros_actuales_usd, salario_mensual_usd, tasa_ahorro_pct,
 ...
 params <- construir_params(ahorros_actuales_usd, salario_mensual_usd, tasa_ahorro_pct,
  ...
  cat("@@BLURB@@texto=Cada barra agrupa simulaciones según cuánto más (o menos) patrimonio terminaste teniendo por comprar en vez de alquilar. A la derecha de la línea roja, comprar te dejó con más plata en ese escenario; a la izquierda, te fue mejor alquilando.@@\n")
  g_dif <- ggplot(diferencias, aes(x = diferencia / 1000)) +
    geom_histogram(fill = "#6c5ce7", alpha = 0.8, bins = 40) +
    geom_vline(xintercept = 0, linetype = "solid", color = "#ff6b6b", linewidth = 1.2) +
    geom_vline(xintercept = media_dif / 1000, linetype = "dashed", color = "#7fc1ec", linewidth = 1) +
    labs(title = "¿Quién te conviene más: comprar o alquilar?",
         subtitle = sprintf("A la derecha del 0 = te convino comprar (%.0f%% de los casos) | A la izquierda = te convino alquilar (%.0f%% de los casos)", pct_compra_mejor, 100-pct_compra_mejor),
  ...
  cat("@@BLURB@@texto=Este gráfico muestra cómo crecería tu patrimonio a lo largo de los años en cada opción. La línea es el resultado más típico, y las bandas muestran qué tan distintos pueden ser los resultados según cómo se comporte la economía en el camino.@@\n")
  g_evol <- ggplot(patrimonio_por_año, aes(x = año, color = tipo_clean, fill = tipo_clean)) +
    ...
    scale_color_manual(values = c("Compra" = "#7fc1ec", "Alquiler" = "#6fe0a0")) +
    scale_fill_manual(values = c("Compra" = "#7fc1ec", "Alquiler" = "#6fe0a0")) +
    scale_y_continuous(labels = scales::label_dollar(suffix = "K")) +
    labs(title = "¿Cómo evoluciona tu patrimonio con el tiempo?", subtitle = "Línea = resultado típico | Banda oscura = la mitad de los casos | Banda clara = la gran mayoría de los casos",
...
(function () {
  ...
  function renderBlurbCard(f) {
    return '<div class="rs-blurb">' + f.texto + "</div>";
  }
